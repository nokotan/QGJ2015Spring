...
struct CollisionCheckHelper {
	...
	static void CollisionCheck(Type& obj, IMapTileLayor<MapType>& layor, IsTileEmpty EmptyCheck) {
		...
		int first_X = static_cast<int>(floor(rectX.X / static_cast<double>(layor.GetTileSize())));
		int last_X = static_cast<int>((rectX.X + rectX.Width - 1) / static_cast<double>(layor.GetTileSize()));
		int first_Y = static_cast<int>(floor(rectX.Y / static_cast<double>(layor.GetTileSize())));
		int last_Y = static_cast<int>((rectX.Y + rectX.Height - 1) / static_cast<double>(layor.GetTileSize()));
		...
		for (int i = first_X; i <= last_X; i++) {
			for (int j = first_Y; j <= last_Y; j++) {
				...
				if (!is_empty && rectX.IntersectWith(tilerect)) {
					...
					OnCollideFromSideCaller<Type, MapType>::OnCollideFromSide(obj, tile);
				}
			}
		}
		...
		first_X = static_cast<int>(floor(rectY.X / static_cast<double>(layor.GetTileSize())));
		last_X = static_cast<int>((rectY.X + rectY.Width - 1) / static_cast<double>(layor.GetTileSize()));
		first_Y = static_cast<int>(floor(rectY.Y / static_cast<double>(layor.GetTileSize())));
		last_Y = static_cast<int>((rectY.Y + rectY.Height - 1) / static_cast<double>(layor.GetTileSize()));
			...
			for (int j = first_Y; j <= last_Y; j++) {
				...
				if (!is_empty && rectY.IntersectWith(tilerect)) {
					if (obj.dy > 0) {
						...
						OnCollideFromBottomCaller<Type, MapType>::OnCollideFromBottom(obj, tile);
					} else if (obj.dy < 0) {
