...
public:
	...
	/// <summary>X軸方向の速度を取得します。</summary>
	virtual Pixel GetDeltaX() { return dx; };
	/// <summary>Y軸方向の速度を取得します。</summary>
	virtual Pixel GetDeltaY() { return dy; };
	...
	/// <summary>タイルマップをコピーします。</summary>
	/// <param name="right">コピー元のタイルマップ</param>
 	Map& operator=(Map& right) {
		// 自身に代入しようとするときと、コピー元が空の場合は何もしない。
		if (&right == this) {
			return *this;
		}

		Create(right.Width, right.Height, right.Tiles, true);
		return *this;
	};	
